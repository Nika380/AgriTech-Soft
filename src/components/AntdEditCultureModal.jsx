...
const AntdAddCultureForm = ({ id, data, setOpenModal }) => {
  ...
  const onFinish = async (values) => {
    setLoading(true);
    console.log(values);
    await API.put(`/cultures/update-info/${data.id}`, { values }).then(
      (res) => {
        form.resetFields();
        setOpenModal(false)
  ...
  return (
    <div style={{ height: "45vh" }} className="p-12">
      <Form form={form} onFinish={onFinish}>
        <Form.Item name={"cultureName"} initialValue={data.cultureName}>
        ...
        <Form.Item name={"squareMeter"} initialValue={data.squareMeter}>
          <Input type="number" step={"0.1"} placeholder="კვ. მ" />
        </Form.Item>
        {/* <Form.Item name={"agrocalendar"} initialValue={data.agrocalendar}>
          <Select
            options={optionList}
